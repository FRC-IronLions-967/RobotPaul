package frc.robot;

/**
 * The RobotMap is a mapping from the ports sensors and actuators are wired into
 * to a variable name. This provides flexibility changing wiring, makes checking
 * the wiring easier and significantly reduces the number of magic numbers
 * floating around.
 */
public class RobotMap {
  
  // public static int driveLeftMaster = 0;
  // public static int driveLeftFollowerA = 1;
  // public static int driveLeftFollowerB = 2;
  
  // public static int driveRightMaster = 3;
  // public static int driveRightFollowerA = 4;
  // public static int driveRightFollowerB = 5;
  
  public static int driveLeftMaster = 7;
  public static int driveLeftFollowerA = 8;
  public static int driveLeftFollowerB = 11;
  
  public static int driveRightMaster = 4;
  public static int driveRightFollowerA = 3;
  public static int driveRightFollowerB = 12;
}
